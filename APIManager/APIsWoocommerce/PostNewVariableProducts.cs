...
{
    class PostNewVariableProducts
    {
        public static void Woocommercepostnewproducts()
        {
            string sku1 = "";
            Console.WriteLine("conectar a base dados...");
            SqlConnection conn = new("Data Source=192.168.1.15;Initial Catalog=TESTE_Ricardo;Integrated Security=FALSE;User ID=ricardo;password=!kEp7.9#nd; MultipleActiveResultSets=true");
            //public SqlConnection conn = new SqlConnection("Data Source=192.168.1.15;Initial Catalog=TESTE_Ricardo;Integrated Security=SSPI; MultipleActiveResultSets=true");
            ...
            try
            {
		        ...
		        where [TESTE_Ricardo].[dbo].woocommerceimg.sku = '" + sku1 + "'";
                    ...
                    conn.Close();
                    //string res23 = string.Join(",", dataListsrc.Select(x => x.ToString()).ToArray());
                    //dataListsrc.ForEach(Console.WriteLine);
                    //Console.WriteLine(res23);

                    /*query = @"SELECT * FROM
              [TESTE_Ricardo].[dbo].woocommerceatt
              where [TESTE_Ricardo].[dbo].woocommerceatt.sku = '" + sku1 + "'";
                    SqlCommand cmd4 = new SqlCommand(query, conn);
                    conn.Open();
                    var dataReader4 = cmd4.ExecuteReader();
                    dataReader4.Read();
                    int idatt1 = 1;
                    int idatt2 = 2;
                    string attributeVisible = dataReader4["visible"].ToString();
                    string attributeVariation = dataReader4["variation"].ToString();
                    conn.Close();
                    cmd4.Dispose();
                    dataReader4.Close();*/
                ...
                [TESTE_Ricardo].[dbo].woocommerceatt.idatt = 2";
                    ...
                    Console.WriteLine("conectar ao website para inserir produtos");
                    System.Threading.Thread.Sleep(2000);
                    Console.Write(Environment.NewLine);
                    try
                    {
                        using (var wb = new WebClient())
                        {
                            ...
                            //var response = wb.UploadFile($"https://teste.levira.de/wp-json/wc/v3/products?consumer_key=ck_fa1a36b0a36bdc1ee790a8105283bc43814498e0&consumer_secret=cs_d94165dec78f4cc6ce4fe33c49cf85219567c33a", @"C:\Users\ricardo.machado\Documents\filesjson\apiwoocommerce.json");
                            var response = wb.UploadString($"https://teste.levira.de/wp-json/wc/v3/products?consumer_key=ck_fa1a36b0a36bdc1ee790a8105283bc43814498e0&consumer_secret=cs_d94165dec78f4cc6ce4fe33c49cf85219567c33a", output);
                        }
                    ...
                    catch (WebException ex)
                    {
                        if (ex.Response != null)
                        {
                            string path2 = @"C:\Users\ricardo.machado\Documents\filesjson\logwebserviceerror.txt";
                    ...
                    //(new WebClient()).UploadStringAsync(new Uri($"https://teste.levira.de/wp-json/wc/v3/products?consumer_key=ck_fa1a36b0a36bdc1ee790a8105283bc43814498e0&consumer_secret=cs_d94165dec78f4cc6ce4fe33c49cf85219567c33a"), result);
            ...
            {
                string path3 = @"C:\Users\ricardo.machado\Documents\filesjson\logerrors.txt";
